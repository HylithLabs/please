...
fn get_manual(cmd: &str) -> Option<&'static str> {
    match cmd {
        ...
        "run" => Some(
            "NAME\n    please run - figure out how to run this project\n\nSYNOPSIS\n    please run [--here | --new-terminal]\n    please run set [--term] [--check \"<probe>\"] \"<command>\" [\"<command>\" ...]\n    please run reset\n\nDESCRIPTION\n    Reads a small set of manifest and doc files (README, package.json, Cargo.toml, Dockerfile, and similar), asks the configured AI provider how to start the project, then asks for confirmation before running it. The decision is saved outside the working tree, so it never shows up in git status, and every run after that replays it directly with no AI call.\n\n    A dev server or watcher opens in a new terminal window so your shell stays free. Pass --here to force it inline, or --new-terminal to force a new window.\n\n    `please run set` records the commands by hand with no AI call - the way `please chat` overrides how a project runs. --term marks it long-running; --check gives a probe command that must succeed first. `please run reset` clears the saved decision.",
        ),
        "update" => Some(
